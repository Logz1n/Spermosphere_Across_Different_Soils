...
```{r}
###Bacterial RDS
BSpermosphere_RDS_CSS <- readRDS("Bacteria_RDS/Spermosphere.rds") ##Using Normalized Reads

BSpermosphere_RDS_NoNorm <- readRDS("Bacteria_RDS/Spermosphere_bac_clean.rds") ##Using Non-normalized Reads

BSpermosphere_RDS_NoNorm@sam_data$State_County <- interaction(BSpermosphere_RDS_NoNorm@sam_data$State, BSpermosphere_RDS_NoNorm@sam_data$County)
...
stc <- unique(BSpermosphere_RDS_NoNorm@sam_data$State_County) 
...
# Create Bray distance matrix
bac.bray.0 = phyloseq::distance(bac_sperm_0, "bray")
...
beta.disp.0b <- betadisper(bac.bray.0, bac_sperm_0@sam_data$Type)

# Display PERMANOVA results
adonis.output.0

# Perform permutation test for beta dispersion
permutest(beta.disp.0)#for Treatment
permutest(beta.disp.0b)# for type
...
# Create Bray distance matrix
bac.bray.17 = phyloseq::distance(bac_sperm_17, "bray")
...
beta.disp.17.brayb <- betadisper(bac.bray.17, bac_sperm_17@sam_data$Type)
# Display PERMANOVA results
adonis.output.17
...
# Create Bray distance matrix
bac.bray.V2 = phyloseq::distance(bac_sperm_V2, "bray")
...
beta.disp.V2b <- betadisper(bac.bray.V2, bac_sperm_V2@sam_data$Type)
# Display PERMANOVA results
adonis.output.V2

# Perform permutation test for beta dispersion
permutest(beta.disp.V2)
permutest(beta.disp.V2b)
# Perform ordination using MDS with Bray distance
...
beta.disp.0.unib <- betadisper(prok.dist.uni.0hrs, bac_sperm_0@sam_data$Type)
...
# PERMANOVA - testing for differences in centroids
set.seed(12345)  # Set seed for reproducibility
prok.dist.uni.17hrs = UniFrac(bac_sperm_17, weighted = TRUE)  # Create distance matrix
adonis.17.uni <- adonis2(prok.dist.uni.17hrs ~ Trt * Type, as(sample_data(bac_sperm_17), "data.frame"))  # Perform PERMANOVA to test for significant changes
adonis.17.uni  # Display PERMANOVA results
...
# PERMANOVA - testing for differences in centroids
set.seed(12345)  # Set seed for reproducibility
prok.dist.uni.V2hrs = UniFrac(bac_sperm_V2, weighted = TRUE)  # Create distance matrix
adonis.V2.uni <- adonis2(prok.dist.uni.V2hrs ~ Trt * Type, as(sample_data(bac_sperm_V2), "data.frame"))  # Perform PERMANOVA to test for significant changes
adonis.V2.uni  # Display PERMANOVA results
...
beta.disp.V2.unib <- betadisper(prok.dist.uni.V2hrs, bac_sperm_V2@sam_data$Type)
permutest(beta.disp.V2.unib)
