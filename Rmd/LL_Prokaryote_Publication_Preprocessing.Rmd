...
```{r}
# Metadata #
samp_dat_bac <- read.csv("Spermosphere_16S_Metadata.csv", na.strings = "na")
...
otu_bac <- read.csv("otu_table_16S_Bacteria.txt", sep = "\t") #txt file is tab separated
...
# Taxonomy #
taxonomy.bac <- read.csv("16s_taxonomy_SINTAX.txt", sep = "")
...
FASTA.bac <- readDNAStringSet("otus.fasta", format="fasta", seek.first.rec=TRUE, use.names=TRUE)

# Phylogentic tree #
tree <- phyloseq::read_tree("otu_tree.tre")
...
#save(bac.unedited, file = "physeq_bac_full.RData")

# Remove singletons #
bac.unedited <- filter_taxa(bac.unedited, function (x) {sum(x > 0) >1}, prune=TRUE)
#save objects to reduce memory?
#save(bac.unedited, file = "physeq_bac.RData")
load("physeq_bac.RData")
```
...
bac_mock <- bac_no_chloro %>% 
  ...
  phyloseq::filter_taxa(function(x) sum(x) > 10, TRUE) # filter OTUs to have more than 1 read in mock samples
...
saveRDS(bac_sperm, file = "Spermosphere_bac_clean.rds")
```

```{r}
# Restore the object
bac_sperm <- readRDS(file = "Spermosphere_bac_clean.rds")
```
```{r}
###### READS PER SAMPLE ######
